...
const TemplateTableModal: React.FC<ITemplateTableModal> = (
  ...
  const addTemplate = (e: React.FormEvent) => {
    ...
    workWithServer.addTableTemplate(template).then(() => {
      changeTableTemplateData()
      setShow(false)
      dispatch(addAlert({
        type: 'success',
  ...
  const updateTemplate = (e: React.FormEvent) => {
    e.preventDefault()
    workWithServer.updateTableTemplate(template).then(() => {
      changeTableTemplateData()
      setShow(false)
      dispatch(addAlert({
        type: 'success',
  ...
  const notSelectedCols = allCols
    .filter(el => !(template.tableCols && template.tableCols.find(col => col.col === el.accessor)))
          ...
          <div className="mx-auto flex">
            ...
            />
            {
              template.id && <Button
                  ...
                  text="Удалить"
                  onClick={() => {
                    workWithServer.deleteTableTemplate({id: template.id})
                      .then(data => {
